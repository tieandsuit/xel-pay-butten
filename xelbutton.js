	var defaults = {
		...
		//       now they are also defined in config.json
 		recipient: 0,
		server: 'http://nxt.sx:2000/xelbutton.php',
		nrs: 'http://xel.org:17876',
                amount: '1.0',
		fee: '0.1',
		title: 'Sample XEL payment',

		// images
		mainButton: 'images/pay_with_xel_blue.png',
		payButton: 'images/pay_button.png',
		okButton: 'images/ok_button.png',
		confirmButton: 'images/confirm_button.png',
	...
	$.fn.xelbutton = function(config) {
		...
		div.config = $.extend({}, defaults, config);
			...
			+ 'onclick="$(\''+this.selector+'\').xelbuttonOpenPaymetWindow()" '
		...
		div.innerHTML = div.innerHTML + "<div id='"+div.id+"_panel_outer_div' class='dim-panel' "
			...
			+ "&nbsp;&nbsp;&nbsp;&nbsp;FEE <input id='"+div.id+"_fee' type='text' value='"+div.config.fee+"' size='8' placeholder='XEL' class='fee' /></p>"
			+ "<p><input id='"+div.id+"_secret_phase' type='password' size='70' placeholder='Secret Phrase' class='secret' />"
			+ "<img id='"+div.id+"_button' onclick=\"$('"+div.id+"').xelbuttonSendTransaction()\" class='submit' />"
			+ "</p></div>"
			+ "<p class='small-text'>The transaction is signed locally, the password does not leave your computer."
			+ "<br />"
			+ "<br /> Merchant XEL account: " + div.config.recipient + ""
			+ "<br />"
			+ "<br />NODE :  " +div.config.nrs + "</p>"
	...
	$.fn.xelbuttonMessage = function(message) {
		...
		$('#'+div.attr('id')+'_message_inner_div').html(message+"<br><img onclick=\"$('#"+div.attr('id')+"').xelbuttonHide();\" alt=\"\" src=\"" + defaults.okButton + "\" /></div></div>");
	};
	...
	$.fn.xelbuttonSendTransaction = function() {
		...
		if (secretPhrase.length == 0) {
			$('#'+this.selector).xelbuttonMessage("Please enter your secret phrase");
			return;
		}
		...
		CrossDomainRequest(request,
			function(data) {
				...
				if (data.errorDescription) {
					...
					console.info('xelbutton: '+data.errorDescription);
					return;
				}
...
0065cd1d
				...
				CrossDomainRequest(div.config.server+'?requestType=broadcastTransaction&transactionBytes='+transactionBytes, function (r) {
					...
					console.info('xelbutton: Transaction complete!');
				});
			}
		);
	}
